...
async function getSharedCardsForUser(userId) {
	try {
		const user = getUserById(userId);
		const userCards = [];
		const cards = await getAllSharedCards();
		cards.forEach((card) => {
			if (user.cards.findIndex((cardId) => card.id == cardId) !== -1) {
				console.log(card);
				userCards.push(card);
...
async function shareCard(cardId, receiverId) {
	...
	try {
		firebasedb.set(
			firebasedb.ref(db, `users/${receiverId}/sharedCards/`),
			sharedCards
		);
		return true;
	} catch (err) {
...
async function getAllSharedCards() {
	return sharedCards;
}
