...
const FeedCard = (props:any) => {
    ...
    return(
        <Grid item key ={props.key} className="feedCard" style={{width:'80%', margin:'10px 20px', padding:'20px', position:'relative', backgroundColor:'#EEEEEE', display:'flex', flexDirection:'row', alignItems:'center', justifyContent:'space-between'}}>
            <Box className='Left'>
                <Box className='UserInfo' style={{display:'flex', padding:'20px', flexDirection:'row', alignItems:'center', position:'absolute', top:0, left:0}}>
                    ...
                    <Box style={{backgroundColor:'#E46A6A',objectFit:'cover', width:'50px', height:'50px', borderRadius:'50%'}}></Box>}
                    <Typography style={{color:'black', padding:'0px 10px'}}>@{user.username}</Typography>
                ...
                <Box className="PostActions" style={{display:'flex', flexDirection:'row', position:'absolute', bottom:0, left:0, padding:'20px'}}>
                    {credential? 
                    <Box style={{zIndex:99999, display:'flex', flexDirection:'row', alignItems:'center'}}>
                        <BiUpArrow size={22} color={isLiked ? '#02F9A7':'red'} onClick={setLike}/>
            ...
            <Box className="Right">
                <Box className="PostAssets">
                    {credential ? <img onClick={()=>{navigate(`/credential/${credential.id}`)}} style={{height:'250px', width:'250px', borderRadius:'20px'}} src={imageUrl} alt="token"/> : null}
                </Box>
            </Box>
        </Grid>
    )
